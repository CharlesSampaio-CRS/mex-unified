...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const loginWithGoogle = async () => {
    try {
      ...
      console.log('🔐 Iniciando login com Google via EC2...')
      
      // 1. Solicita URL de autenticação do Google via Trading Service (Rust) na EC2
  ...
  const register = async (email: string, password: string, name: string) => {
    ...
    console.log('📥 [AuthContext] Parâmetros:', { email, name, passwordLength: password.length })
    ...
    try {
      ...
      console.log('📝 [REGISTER] Payload:', { email, name, password: '***' })
      ...
      console.log('📊 [REGISTER] Response:', responseText.substring(0, 200))
      let data: any = null
      try {
        data = JSON.parse(responseText)
        console.log('✅ [REGISTER] Response JSON parsed')
      } catch (e) {
        console.log('❌ [REGISTER] Response is not JSON:', responseText)
      }

      if (!response.ok) {
        throw new Error((data && data.error) || responseText || 'Registration failed')
      }
