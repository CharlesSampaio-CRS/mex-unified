...
export function ExchangesManager({ initialTab = 'linked' }: ExchangesManagerProps) {
  ...
  const handleLinkExchange = useCallback(async () => {
    ...
    try {
      ...
      console.log('💾 [Local DB] Salvando exchange no WatermelonDB...')
      
      // 🔐 Encrypt credentials before saving
      console.log('🔐 [Encryption] Encrypting exchange credentials...')
      ...
      console.log('✅ [Local DB] Exchange salva com sucesso:', {
        id: newExchange.id,
        name: selectedExchange.nome,
        userId: user.id
      })
      
      // Também tentar salvar no backend (opcional, pode falhar silenciosamente)
      try {
        ...
        if (response.ok && data.success) {
          console.log('✅ [Backend] Exchange linkada no backend com sucesso')
        } else {
          console.warn('⚠️ [Backend] Falha ao linkar no backend (mas salvo localmente):', data.error)
        }
      } catch (backendErr) {
        console.warn('⚠️ [Backend] Erro ao linkar no backend (mas salvo localmente):', backendErr)
      }
      
      // Fechar modal
      closeConnectModal()
      
      // Pequeno delay para garantir que o WatermelonDB processou
      ...
      console.error('❌ [Local DB] Erro ao salvar exchange:', err)
      alert(t('error.connectExchange'))
    } finally {
      setConnecting(false)
    }
