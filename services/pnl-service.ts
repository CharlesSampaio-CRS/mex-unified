...
class PnLService {
  ...
  private async calculatePnL(
    ...
    const currentValue = currentBalance !== undefined 
      ? currentBalance 
      : (await this.getLatestSnapshot(userId))?.total_usd || 0
    ...
    // Calcula mudança
    const change = currentValue - previousSnapshot.total_usd
    const changePercent = previousSnapshot.total_usd > 0 
      ? (change / previousSnapshot.total_usd) * 100 
      : 0

    return {
      current: currentValue,
      previous: previousSnapshot.total_usd,
      change,
      changePercent,
      period
    }
  ...
  async getTodayPnL(userId: string, currentBalance?: number): Promise<PnLData> {
    ...
    // Calcula mudança
    const change = currentValue - yesterdaySnapshot.total_usd
    const changePercent = yesterdaySnapshot.total_usd > 0 
      ? (change / yesterdaySnapshot.total_usd) * 100 
      : 0

    return {
      current: currentValue,
      previous: yesterdaySnapshot.total_usd,
      change,
      changePercent,
      period: 'today'
    }
  ...
  async getSnapshotsInRange(
    ...
    return await table<BalanceSnapshot>('balance_snapshots')
      .where('user_id', '=', userId)
  ...
  async getEvolutionData(userId: string, days: number = 7): Promise<{
    ...
    if (snapshots.length >= days) {
      return {
        values_usd: snapshots.map(s => s.total_usd),
    ...
    for (let i = 0; i < days; i++) {
      ...
      if (beforeSnapshot && afterSnapshot && beforeSnapshot.id !== afterSnapshot.id) {
        ...
        const valueDiff = afterSnapshot.total_usd - beforeSnapshot.total_usd
        const ratio = (targetTimestamp - beforeSnapshot.timestamp) / timeDiff
        const interpolatedValue = beforeSnapshot.total_usd + (valueDiff * ratio)
        values_usd.push(interpolatedValue)
      } else if (beforeSnapshot) {
        // Usa o valor mais próximo antes
        values_usd.push(beforeSnapshot.total_usd)
      } else if (afterSnapshot) {
        // Usa o valor mais próximo depois
        values_usd.push(afterSnapshot.total_usd)
      } else {
        // Fallback: usa o primeiro snapshot disponível
        values_usd.push(snapshots[0].total_usd)
      }
    }
